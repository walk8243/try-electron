...
const AppInfo = () => {
  ...
  return (
    <Box component='section' my={3}>
      <Heading level={2} hidden>アプリケーションの情報</Heading>
      ...
      <Grid container alignItems='center' gap={2} sx={{ px: 1 }}>
        ...
        <Grid container item direction='column' xs zeroMinWidth gap={2}>
          ...
          <Grid item>
            <Link component='button' onClick={() => handleOpenSource('https://github.com/walk8243/amethyst-electron')}>
...
const OssList = () => (
  <Box component='section' my={3}>
    <Heading level={2} hidden>使用しているOSSの一覧</Heading>

    <TableContainer component={Paper}>
      <Table>
        <TableHead>
          {OssInfo({ name: 'OSS', version: 'Version'})}
        </TableHead>
        <TableBody>
          {OssData.map((data, index) => <OssInfo key={index} {...data} />)}
        </TableBody>
      </Table>
    </TableContainer>
  </Box>
...
const ButtonArea = () => {
  ...
  return (
    <Box component='section' my={3}>
      <Heading level={2} hidden>ボタン配置領域</Heading>
...
const CopyRight = () => (
  <Box component='section' mt={3}>
    <Heading level={2} hidden>著作権情報</Heading>

    <Grid container justifyContent='center'>
      <Grid item>
        <Typography variant='caption' sx={{ '::before': { content: '"©"', mr: 0.5 } }}>walk8243</Typography>
      </Grid>
    </Grid>
  </Box>
)
